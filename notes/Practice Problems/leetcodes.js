...
split the values into letter vs number
letter: [a,b,c]
num: [1,2]
alternate putting them together into a string

return empty string:
if all letters or nums
if letters length is greater or less than nums length by 2 or more
*/
...
var reformat = function (s) {
  ...
  for (let val of s) {
    ...
    if (!isNaN(Number(val)) === true) {
      organized.nums.push(val);

    } else {
      organized.letters.push(val);
    }

  }

  if (Math.abs(organized.letters.length - organized.nums.length) > 1) {
    return "";
  }

  let output = "";

  const totalLen = organized.letters.length + organized.nums.length;

  let startingVal = "";
  if (organized.letters.length >= organized.nums.length) {
    startingVal = "letters";
  } else {
    startingVal = "nums";
  }
  ...
  let counter = totalLen;


  while (counter > 0) {
    if (startingVal === "letters") {
      output += (organized.letters.pop());
      startingVal = "nums";
    } else {
      output += (organized.nums.pop());
      startingVal = "letters";
    }
